...
-- table to ensure consistency of contact tracing data w.r.t employee with fever and close contacts
CREATE TABLE Blacklist_Employees
       (
              eid   INT REFERENCES Employees(eid)
            , sDate DATE
            , eDate DATE
			, PRIMARY KEY (eid, sDate, eDate)
       )
;
...
CREATE OR REPLACE VIEW session_pax AS
SELECT
       count(sessions.participant_eid) AS pax
